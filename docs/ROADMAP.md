...
- 🟢 **AI Assistant** _(2026-05-19)_ — Desktop-only right-hand sidebar wired to Anthropic / OpenAI / Ollama via the Vercel AI SDK. Per-provider tabs, persisted conversations, streaming responses, a confirm-gated tool catalog with read/write access to the workspace and active editor, and `safeStorage`-encrypted API keys in `~/.mkeditor/assistant.json`. Web build hides the sidebar / Navbar toggle / Settings tab entirely (no localStorage AI keys). Eight-phase delivery covered in [AI_ASSISTANT.md](AI_ASSISTANT.md); end-to-end surface in [ARCHITECTURE.md §4.14](ARCHITECTURE.md).
