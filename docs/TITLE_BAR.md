...
| Window controls       | Drawn in HTML inside `<TitleBar>`. Four new `to:window:*` IPC channels: `minimize`, `maximize`, `close`, `fullscreen`. Maximize button toggles between maximize/restore based on the latest `from:window:state` event. Fullscreen is added in P2 because `role: 'togglefullscreen'` accelerators don't fire on Windows/Linux once `Menu.setApplicationMenu(null)` runs.                                                                                                                                                                                                                                                                                            |
| Renderer menu actions | `dispatchMenuActionExternal` (registered by `<App>`) translates a `MenuAction` into the right effect. `kind: 'channel'` either opens a React modal (`from:modal:open`) or proxies to existing renderer-side handlers via BridgeManager. `kind: 'role'` runs Monaco's `editor.trigger(...)` for undo/redo/cut/copy/paste; uses `to:window:fullscreen` for `togglefullscreen` and `to:window:close` for `quit`. `kind: 'command'` sends `to:command:run` with the `commandId` payload; main's `AppMenu.runCommand` handles it (same dispatch table the native menu uses).                                                                                            |
...
src/browser/
├── core/
│   ├── BridgeManager.ts               MODIFIED — window-control + maximize observable; menuFile*/menuFolder*/menuCommandPalette helpers consumed by both BridgeListeners and the in-window menu
│   └── BridgeListeners.ts             MODIFIED — file/folder/palette receivers delegate to BridgeManager.menu* helpers; new from:window:state handler
├── menuDispatch.ts                    NEW — module-level seam (dispatchMenuActionExternal / registerMenuActionDispatcher); same pattern as openModalExternal
├── index.ts                           MODIFIED — derive `platform` from window.mked.platform and thread it through Managers
├── react/
│   ├── components/
...
│   ├── contexts/
│   │   ├── ManagersContext.tsx        MODIFIED — added `platform` field for React-side OS branching
│   │   └── WindowContext.tsx          NEW — exposes isMaximized + minimize/maximize/close/toggleFullscreen
│   └── App.tsx                        MODIFIED — mount <TitleBar> above <Navbar>; <MenuActionBridge> sentinel registers dispatchMenuActionExternal
...
1. **Create [src/browser/react/contexts/WindowContext.tsx](../src/browser/react/contexts/WindowContext.tsx)** — exposes `{ isMaximized, minimize, maximize, close, toggleFullscreen }`. Subscribes to BridgeManager's window state observable (added in this phase to BridgeManager). Falls back to no-op on web.
2. **Add window-control surface to [src/browser/core/BridgeManager.ts](../src/browser/core/BridgeManager.ts)** — `subscribe/getSnapshot` for `isMaximized`, plus `windowMinimize() / windowMaximize() / windowClose() / windowToggleFullscreen()` and `runCommand(commandId)` methods that proxy to the bridge.
